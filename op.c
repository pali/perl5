...
S_pmtrans(pTHX_ OP *o, OP *expr, OP *repl)
{
    ...
    /* khw doesn't know how these could differ in UTF-8ness */
     ...
     * byte length.  As it constructs the mappings, it carves these up into
     * smaller chunks, but doesn't merge any together.  This makes it easy to
     * find the instances it's looking for.  A second pass is done after this
     * has been determined which merges things together to shrink the table for
     * runtime.  For ASCII platforms, the table is trivial, given below, and
     * uses the fundamental characteristics of UTF-8 to construct the values.
     * For EBCDIC, it isn't so, and we rely on a table constructed by the perl
     * script that generates these kinds of things */
#ifndef EBCDIC
...
#  endif
    ...
    if (complement) {
        ...
        DEBUG_U(PerlIO_printf(Perl_debug_log, "%d: tstr=%s\n", __LINE__, _byte_dump_string(t, tend - t, 0)));
        DEBUG_U(PerlIO_printf(Perl_debug_log, "rstr=%s\n", _byte_dump_string(r, rend - r, 0)));
        ...
        /*DEBUG_U(sv_dump(inverted_tlist));*/
        ...
        while (invlist_iternext(inverted_tlist, &start, &end)) {
            ...
            /* I no longer remember why IV_MAX */
    ...
    /* Parse the input in two passes.  The first pass is to determine if the
     * transliteration can be done in place.  Its output could be used, but
     * generally would be larger and slower to run than the output of the
     * second pass */
    for (pass2 = 0; pass2 < 2; pass2++) {

        /* Here, the input has been adjusted if necessary.  Now create the
         * inversion map.  Initially, everything is TR_UNLISTED.  An inversion
         * list is created, and then we use how the internals work to create
         * the partitioned (by byte length) inversion map */
        ...
        /* And the mapping of each of the ranges is initialized */
        for (i = 0; i < len; i++) {
            r_map[i] = TR_UNLISTED;
        }

        DEBUG_U(PerlIO_printf(Perl_debug_log, "%d: /c=%d; /s=%d; /d=%d\n", __LINE__, complement, squash, del));
        DEBUG_U(invmap_dump(t_invlist, r_map));
        DEBUG_U(PerlIO_printf(Perl_debug_log, "tstr=%s\n", _byte_dump_string(t, tend - t, 0)));
        DEBUG_U(PerlIO_printf(Perl_debug_log, "rstr=%s\n", _byte_dump_string(r, rend - r, 0)));
         ...
         * infinity.
         *
         * This routine makes a couple of modifications to traditional
         * inversion maps.  Two mappings are reserved:
         *
        ...
        *      map for this.  But things like
        ...
        while (t < tend || t_range_count > 0) {
            ...
            if (r_range_count <= 0) {
                if (r >= rend) {
                    ...
                    if (! del) {
                        DEBUG_Uv(PerlIO_printf(Perl_debug_log, "final_map =%" UVXf "\n", final_map));
                    }
                }
                else {
                    if (! rstr_utf8) {
                    ...
                    /* ? This may no longer be needed, but I suspecit it is XXX
                     * */
                    if (! pass2 && r_cp == TR_SPECIAL_HANDLING) {
                        r_range_count = t_range_count;
                    }
             ...
             * and the remainder of the range in element [i].  (That range will
             * have everything in it be of the same class; we can't cross into
             * another class.)  'min_range_count' already contains the smallest
             * of the first two values.  The final one is irrelevant if the map
             * is to the special indicator */

            invmap_range_length_remaining = ((Size_t) i + 1 < len)
                                            ? t_array[i+1] - t_cp
                                            : IV_MAX - t_cp;
            span = MAX(1, MIN(min_range_count, invmap_range_length_remaining));
            /* The MAX was added to fix a bug, maybe the bug came from SPECIAL
             * HANDLING range, which might be clearer to use instead XXX */
            assert(span > 0);
            ...
            if (r_map[i] == TR_UNLISTED) {
                DEBUG_Uv(PerlIO_printf(Perl_debug_log, "Processing %" UVxf "-%" UVxf " => %" UVxf "-%" UVxf "\n", t_cp, t_cp_end, r_cp, r_cp_end));
                 ...
                 *
                 * The t_invlist has been partitioned so that all elements in
                 * any single range have the same number of bytes in their
                 * UTF-8 representations.  And the r space is either a single
                 * byte, or a range of strictly monotonically increasing code
                 * points.  So the final element in the range will be
                 * represented by no fewer bytes than the initial one.  That
                 * means that if the final code point in the t range has at
                 * least as many bytes as the final code point in the r, then
                 * all code points in the t range have at least as many bytes
                 * as their corresponding r range element.  But if that's not
                 * true, the transliteration of at least the final code point
                 * grows in length.  As an example, suppose we had
                 *      tr/\x{fff0}-\x{fff1}/\x{ffff}-\x{10000}/
                 * The UTF-8 for all but the final character occupies 3 bytes
                 * on ASCII platforms.  We have deliberately set up the data
                 * structure so that any range in the lhs gets split into
                 * chunks for processing, such that every code point in a chunk
                 * has the same number of UTF-8 bytes.  We only have to check
                 * the final code point in the rhs against any code point in
                 * the lhs. */
                if ( ! pass2
                    && r_cp_end != TR_SPECIAL_HANDLING
                    && UVCHR_SKIP(t_cp_end) < UVCHR_SKIP(r_cp_end))
                {
                    ...
                    if (ratio > max_expansion) {
                        max_expansion = ratio;
                        DEBUG_U(PerlIO_printf(Perl_debug_log, "New expansion factor: %" NVgf "\n", max_expansion));
                    }
                ...
                if (t_cp == t_array[i]) {
                     ...
                     * merged with the range below.  EXCEPT, we don't merge
                     * ranges whose UTF-8 byte representations have different
                     * lengths, so that we can more easily detect if a
                     * replacement is longer than the source, that is if it
                     * 'grows'.  But in the 2nd pass, after we've already found
                     * out that it grows, there's no reason to not merge */
                    if (   (i > 0 && (   pass2
                ...
                if (merge_with_range_below && merge_with_range_above) {
                     ...
                     * [-1]     Z  -1   # Z => default; as do Z+1, ... infinity
                     */
                    /*t_array[i] = t_cp  I noticed this was missing XXX , But I now think this is right*/
                    r_map[i] = r_cp;
                }
                    ...
                    DEBUG_Uv(PerlIO_printf(Perl_debug_log, "Before fixing up: len=%d, i=%d\n", (int) len, (int) i));
                    DEBUG_Uv(invmap_dump(t_invlist, r_map));
                ...
                DEBUG_Uv(PerlIO_printf(Perl_debug_log, "After iteration: span=%" IVdf ", t_range_count=%" IVdf ", r_range_count=%" IVdf "\n", span, t_range_count, r_range_count));
                DEBUG_Uv(invmap_dump(t_invlist, r_map));
        ...
        if (pass2) {
             ...
             * anything left over in the replacement list.  So, just assume
             * it's one byte per character */
            if (rend > r) {
                r_count++;
            }
        }
    ...
    DEBUG_U(PerlIO_printf(Perl_debug_log, "After everything: \n"));
    DEBUG_U(invmap_dump(t_invlist, r_map));
    ...
    if (!del && !squash) {
        ...
        /* Look to see if the lhs and rhs are identical.  They are if they
         * point to same address, or if everything maps to UNLISTED or to
         * itself.  This catches things that not looking at the normalized
         * inversion map doesn't catch, like tr/aa/ab/ or
    ...
    /* tr/0-ff/58/c  causes a utf-8 to be generated even though neither input was XXX */
    if (   can_force_utf8
        || (   len > 0
            && t_array[len-1] > 255
            && ! (t_array[len-1] == 256 && r_map[len-1] == TR_UNLISTED)))
    {
        SV* r_map_sv;
	...
	invmap = MUTABLE_AV(newAV());
        ...
        */
        ...
        tbl->map[tbl->size] = del ? TR_DELETE : rlen ? final_map : TR_R_EMPTY;

        DEBUG_U(PerlIO_printf(Perl_debug_log,"%s: %d\n", __FILE__, __LINE__));
        for (i = 0; i < tbl->size; i++) {
            if (tbl->map[i] < 0) {
                DEBUG_U(PerlIO_printf(Perl_debug_log," %02x=>%d", i, tbl->map[i]));
            }
            else {
                DEBUG_U(PerlIO_printf(Perl_debug_log," %02x=>%02x", i, tbl->map[i]));
            }
            if ((i+1) % 8 == 0 || i + 1 == tbl->size) {
                DEBUG_U(PerlIO_printf(Perl_debug_log,"\n"));
            }
        }
        DEBUG_U(PerlIO_printf(Perl_debug_log,"Final map 0x%x=>%02x", tbl->size, tbl->map[tbl->size]));

        SvREFCNT_dec(t_invlist);
    }

    /* doop can distinguish between types via it being an AV or a PV */
    Safefree(r_map);
...
The inversion map implementation is capable of handling both UTF-8 and
non-UTF-8 cases, but I left in the existing non-UTF-8 implementation, which
uses tables, because I suspect it is faster.  This means that there is extra
code, purely for runtime performance.
...
them in general is that they can become fragmented requiring more space than
you would expect, to represent the underlying data.  However, the typical tr///
would not have this issue, requiring only very short inversion maps to
represent, in some cases shorter than the table implementation.
...
could be implemented without a table and be quite fast.
