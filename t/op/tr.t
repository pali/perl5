# Fails to compile without toke.c fix, so should be a test tr/\x{ff}-\x{104}/\x{100}-\x{105}/
BEGIN {
    chdir 't' if -d 't';
    require './test.pl';
    set_up_inc('../lib');
...
is(length $x, 3);
if ($::IS_ASCII) { # ASCII
    is($x, 256.66.258, __LINE__);
}
else {
    is($x, 256.65.258, __LINE__);
}

# EBCDIC variants of the above tests
($x = 256.193.258) =~ tr/a/b/;
is(length $x, 3, __LINE__);
is($x, 256.193.258, __LINE__);
