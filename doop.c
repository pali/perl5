/*    doop.c
 *    XXX Make sure UTF8SKIP uses are safe
 *
...
STATIC Size_t
S_do_trans_simple(pTHX_ SV * const sv, OPtrans_map * tbl)
{
    Size_t matches = 0;
    STRLEN len;
    ...
    DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: entering trans_simple\n", __FILE__, __LINE__));
    DEBUG_U(sv_dump(sv));
    ...
    DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: returning %" IVdf "\n", __FILE__, __LINE__, matches));
    DEBUG_U(sv_dump(sv));
    return matches;
}
...
STATIC Size_t
S_do_trans_count(pTHX_ SV * const sv, OPtrans_map * tbl)
{
    STRLEN len;
    ...
    DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: count returning %" IVdf "\n", __FILE__, __LINE__, matches));
    return matches;
}
...
STATIC Size_t
S_do_trans_complex(pTHX_ SV * const sv, OPtrans_map * tbl)
{
    STRLEN len;
    U8 *s = (U8*)SvPV_nomg(sv, len);
    U8 * const send = s+len;
    ...
    DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: entering trans_complex\n", __FILE__, __LINE__));
    DEBUG_U(sv_dump(sv));
    if (!SvUTF8(sv)) {
	U8 *d = s;
	U8 * const dstart = d;
...
/* Helper function for do_trans().
 * Handles cases where search and replacement charlists are UTF-8 and
...
STATIC Size_t
S_do_trans_count_utf8(pTHX_ SV * const sv, AV * const invmap)
{
    U8 *s;
    U8 *send;
    Size_t matches = 0;
    ...
    DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: utf8_count returning %" IVdf "\n", __FILE__, __LINE__, matches));
    return matches;
}

/* Helper function for do_trans().
 * Handles cases where search and replacement charlists are UTF-8 and
 * either not identical or flags are present.
 *
 * sv may or may not be utf8.
 */

STATIC Size_t
S_do_trans_utf8(pTHX_ SV * const sv, AV * const invmap)
{
    U8 *s;
    U8 *send;
    U8 *d;
    ...
    s = (U8*)SvPV_nomg(sv, len);
    DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: entering trans_utf8\n", __FILE__, __LINE__));
    DEBUG_U(sv_dump(sv));
    DEBUG_U(invmap_dump(from_invlist, map));

    send = s + len;
    s0 = s;
    DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: len=%" UVuf "; '%s'\n", __FILE__, __LINE__, len, s0));
	...
	dend = d + len;
         ...
         * this particular input string will grow.  Allocating based on the
         * worst case expansion factor of 14 (13 on ASCII platforms) seems way
         * too high for long strings.  So we make an initial guess that the
         * output will be the same length as the input, and adjust if necessary
         * as we go along */
  ...
  restart:
    ...
    while (s < send) {
        ...
        DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: '%s'\n", __FILE__, __LINE__, s));
        /* Get the code point of the next character in the string */
        if (! SvUTF8(sv) || UTF8_IS_INVARIANT(*s)) {
            from = *s;
            s_len = 1;
        ...
        DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: from=%" IVdf "\n", __FILE__, __LINE__, from));
        ...
        to = map[i];
        DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: i=%" IVdf ": %" IVdf "=>%" IVdf "\n", __FILE__, __LINE__, i, from, to));
        ...
        if (! squash || to != previous_map) {
            if (out_is_utf8) {
                ...
                if (to >= 256) {
                    ...
                    d = d0;
                    matches = 0;
                    DEBUG_U(PerlIO_printf(Perl_debug_log, "%s: %d: %" IVdf "=>%" IVdf "\n", __FILE__, __LINE__, from, to));
                    goto restart;
                }
                *d++ = to;
            }
	...
	if (d > dend) {
	    const STRLEN clen = d - d0;
            /* Add F * number of bytes remaining, where F is something like 2 or 3/2 XXX */
	    const STRLEN nlen = dend - d0 + s_len + UTF8_MAXBYTES;
	    if (inplace)
		Perl_croak(aTHX_ "panic: do_trans_utf8 line %d",__LINE__);
	    Renew(d0, nlen + UTF8_MAXBYTES, U8);
	    d = d0 + clen;
	    dend = d0 + nlen;
	}
        s += s_len;
 ...
 * should be an OP_TRANS or OP_TRANSR op, whose op_sv field contains a
 * translation table or inversion map.
...
Perl_do_trans(pTHX_ SV *sv)
{
    ...
    bool use_utf8_fcns = flags & OPpTRANS_TO_UTF;
    bool identical     = flags & OPpTRANS_IDENTICAL;
    ...
    if (use_utf8_fcns) {
        ...
        if (identical) {
            return do_trans_count_utf8(sv, (AV *) map);
        }
        else {
            return do_trans_utf8(sv, (AV *) map);
        }
    }
    else {
